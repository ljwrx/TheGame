...
void VectorManager::_checkAttackList()
{
	...
	while (!_attackList.empty())
	{
		...
		if (node->getKind() & BaseActionSpriteKind::Fighter)
			_attackCheckAndHit(_pDataList.at(node->getDataIndex()).getAttackForce(), node, &_cList);
		else if (node->getKind() & BaseActionSpriteKind::Range)
		{
			if (_particalSystem)
			{
			...
			_attackCheckAndHit(_cDataList.at(node->getDataIndex()).getAttackForce(), node, &_pList);
		_attackList.popBack();
	}
}

void VectorManager::_loadDataFromFileWithData(int s_data, int i_data, int level)
{
	_pDataList;
	_cDataList;
}
